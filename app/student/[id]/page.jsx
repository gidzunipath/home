...
const ApplicantDetail = () => {
  ...
  // Sync tab from URL (?tab=profile kept for older links)
  const tabFromUrl = searchParams.get("tab");
  useEffect(() => {
    const resolvedTab = tabFromUrl === "profile" ? "feedback" : tabFromUrl;
    if (resolvedTab && VALID_CLIENT_TAB_IDS.has(resolvedTab)) {
            ...
            <div className="min-h-[400px] sm:min-h-[600px]">
              ...
              {activeTab === "tasks" &&
                (canAccessVisaSection(applicant?.status) ? (
                  ...
                  <TabAccessDenied
                    title={TAB_ACCESS_DENIED.tasks.title}
                    message={TAB_ACCESS_DENIED.tasks.message}
              ...
              {activeTab === "german-life" &&
                (canAccessGermanLifeSection(applicant?.status) ? (
                  <GermanLifeTab />
                ) : (
                  <TabAccessDenied
                    title={TAB_ACCESS_DENIED.germanLife.title}
                    message={TAB_ACCESS_DENIED.germanLife.message}
                    onContact={() => setShowMessageModal(true)}
                  />
                ))}
              {activeTab === "timeline" && (
